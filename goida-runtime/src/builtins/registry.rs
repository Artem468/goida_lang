pub use crate::ast::prelude::BuiltinTypeSpec;
use crate::ast::prelude::Span;
use crate::interpreter::prelude::{Interpreter, Module, RuntimeClassDefinition, SharedInterner};
...
type ClassInstaller = fn(&SharedInterner) -> (Symbol, SharedMut<RuntimeClassDefinition>);
    ...
    fn install_from(&mut self, registry: &BuiltinRegistry) -> Result<(), Self::Error> {
        ...
        for error in registry.error_classes() {
            ...
            self.module.classes.entry(symbol).or_insert_with(|| {
                SharedMut::new(RuntimeClassDefinition::new_with_base(
                    symbol,
                    base,
                    Span::default(),
                ))
