...
печать("Размер списка: " + размер(числа)); // 3
...
печать("Размер словаря: " + размер(словарь)); // 2
...
| `размер` | `size`/`length` | Получение размера коллекции |
...
- Только целые числа (i64), нет поддержки чисел с плавающей точкой
- Нет модульной системы (модули реализованы частично)
- Нет обработки ошибок времени выполнения
- Нет методов для работы со списками и словарями (добавление, удаление элементов, размер)
